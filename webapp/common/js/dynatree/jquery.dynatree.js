...
DynaTreeNode.prototype = {
	...
	_getInnerHtml: function() {
		...
		if ( data.icon ) {
			if (data.icon.charAt(0) === "/" || (data.icon.charAt(0) === "." && data.icon.charAt(1) === ".")){
				imageSrc = data.icon;
			}else{
...
//			noop(); // keep JSLint happy
		...
		if(!nodeTitle){
			...
			if( opts.noLink || data.noLink ) {
				nodeTitle = '<span style="display:inline-block;" class="dynatree-nolink ' + opts.classNames.title + '"' + tooltip + '>' + data.title + '</span>';
...
// MTREE //////////////////////////////////////////////////////////////////////////////////////////
var MTree = function(id) {
	this.selector = id;
	this.data = [];
	this.nodes = [];

	this.rootNodeParentKey = "_ROOT_";
}
MTree.prototype = {
	add : function(id, pid, name, url, target, icon, open, attrs) {
	...
	, getNodes: function() {
		...
		for(var i=0; i<data.length; i++) {
			var key = data[i].key;
			if(map.hasOwnProperty(key)) {
				data[i]['children'] = map[key];
				data[i]['isFolder'] = true;
			}
		}
		for(var i=0; i<data.length; i++) {
			if(data[i]['pid'] == this.rootNodeParentKey) {
	...
	, getTree : function() {
		return $(this.selector).dynatree("getTree");
	}
	, getRoot : function() {
		return $(this.selector).dynatree("getRoot");
	}
	, getNodeByKey : function(key) {
		return this.getTree().getNodeByKey(key);
	}
	, getNamePath : function(node, sep, def) {
		var path = node.getKeyPath().substring(1).split("/"), names = [];
		var self = this;
		$.each(path, function(idx, key) {
			names.push(self.getTree().getNodeByKey(key).data.title);
		});
		if(def && names.length == 1) return def;
		return names.slice(1).join(sep || ' > ');
	}
	, start : function(opts) {
		$(this.selector).dynatree($.extend({
			  clickFolderMode: 1
			, minExpandLevel : 2
			, autoFocus: false
			, children : this.getNodes()
//			, imagePath : '/js/dynatree/icons/'
			, onActivate: function(node) {
//			, onClick: function(node) {
				node.expand(true);
				if(node.data.url) {
					window.open(node.data.url, node.data.target || '_self');
				}
				node.tree.activeNode.deactivate();
			}
